...
CategoryArrSchema.pre('save', async function (next) {
  ...
  try {
    // 查找最大的 id 值
    const maxId = await mongoose.model('CategoryArr').findOne().sort('-id').select('id').lean();
...
// 创建一个数据模型
const CategoryArr = mongoose.model('CategoryArr', CategoryArrSchema);
